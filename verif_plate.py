...
while True:
    ...
    for license_plate in license_plates.boxes.data.tolist():
        ...
        if car_id != -1:
            ...
            # Check if the license plate was captured before
            if license_plate_text is not None:
                if license_complies_format(license_plate_text):  # Assuming this checks the format
                    print(license_plate_text)

                if license_plate_text in detected_plates:
                    print('License plate captured before: True')
                else:
                    print('License plate captured before: False')
